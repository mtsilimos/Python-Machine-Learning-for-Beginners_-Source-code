...
"""The embeddings from Word2Vec, FastText, GloVe and BERT are all dense, continuous vectors that encode semantic relationships, not raw counts or frequencies. 
Therefore, they are incompatible with Naive Bayes, which is a count-based probabilistic model. While these embeddings can technically be used as input features for a Random Forest model, 
Random Forest is generally not the optimal classifier for leveraging the high-dimensional, semantic information within these embeddings, 
